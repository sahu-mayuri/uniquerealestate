...
export default function BedroomButtonDropdown() {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left w-full group">
      ...
      >
        <div className="py-1">
          {bedroomList.map((list) => <MenuItem>
            <a
              href="#"
              className="block pl-10 text-base font-poppins text-black data-[focus]:bg-blue-500 data-[focus]:text-white" key={list.id} onClick={() => handleBedroomSelect(list)}>
              {list.name}
            </a>
          </MenuItem>)
          }
